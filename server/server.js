...
app.patch("/todos/:id", authenticate, async (req, res) => {
    ...
    app.post("/users", async (req, res) => {
        try {
            ...
            /* Explanation of below:
                ...
                2.) user.generateAuthToken creates an access property and a custom hashed token and pushes it to user's empty tokens array.
                3.) the user is saved once again (in the generateAuthToken method) and the promise is returned that will return the value of "token" so that more can be chained on
                4.) back in server.js, we now have user and token.  token is used by res.header which sends the token back as an HTTP response header and then user is sent back as well.
            */
    ...
    //GET "/users/me" - requires authentication, finds user and returns them
